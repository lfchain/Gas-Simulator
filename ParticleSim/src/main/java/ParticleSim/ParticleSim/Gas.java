...
implements ComponentListener, ActionListener, AdjustmentListener,
  ...
  public void updateHistogram(Graphics realg) {
	...
	g.drawString("P = " + showFormat.format(p), x, fm.getAscent()+2*fm.getHeight());
	g.drawString("kT = " + showFormat.format(temp), x, fm.getAscent()+3*fm.getHeight());
	g.drawString("PV/nkT = " + showFormat.format(p*v/(molCount*temp)), x, fm.getAscent()+4*fm.getHeight());
	g.drawString("P(V-nb)/nkT = " + showFormat.format(p*(v-totalV*vadj)/(molCount*temp)), x, fm.getAscent()+5*fm.getHeight());
	realg.drawImage(dbimage, 0, 0, this);
  }
  */
  ...
  double maxwellDist(double v, double mass) {
	// if (energyCheck.getState())	return Math.exp(-v/temp)/temp;
  ...
  public void adjustmentValueChanged(AdjustmentEvent e) {
	...
	if (e.getSource() == gravityBar)	gravity = gravityBar.getValue() * (.001/20);
	if (e.getSource() == heaterTempBar)	adjustHeaterTemp();
	if (e.getSource() == molCountBar)	adjustMolCount();
	if (e.getSource() == colorBar)		adjustColors();
	*/
  }
  /*
  ...
  void adjustColors() {
	...
	for (i = 0; i != molCount; i++)	setColor(mols[i]);
  }
